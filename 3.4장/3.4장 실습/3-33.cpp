...
void Ground()
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, 3, 512, 343, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, pBytes[1]);
...
void Triangle()
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, 3, 512, 342, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, pBytes[0]);
	glBegin(GL_POLYGON);
	if (bNomal == true)
...
void Circle_Sphere1()
{
	...
	glRotatef(-Circle_Sphere1_Y / 2, 0.0f, 1.0f, 0.0f);
	//glColor3f(1.f, 0.0f, 0.0f);
	glTexImage2D(GL_TEXTURE_2D, 0, 3, 512, 255, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, pBytes[2]);
	// 자동 매핑 설정
	glEnable(GL_TEXTURE_GEN_S);
	glEnable(GL_TEXTURE_GEN_T);
	// 구 매핑
...
{
	//glColor3f(0.0f, 1.0f, 0.0f);
	glTexImage2D(GL_TEXTURE_2D, 0, 3, 1024, 512, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, pBytes[3]);
	// 자동 매핑 설정
	glEnable(GL_TEXTURE_GEN_S);
	glEnable(GL_TEXTURE_GEN_T);
	// 구 매핑
...
GLvoid SetupRC()
{
	...
	pBytes[0] = LoadDIBitmap("3-33 벽돌.bmp", &info);
	pBytes[1] = LoadDIBitmap("3-33 잔디.bmp", &info);
	pBytes[2] = LoadDIBitmap("3-33 지구.bmp", &info);
	pBytes[3] = LoadDIBitmap("3-33 달.bmp", &info);
